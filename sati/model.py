...
class Model:
    ...
    image : ``numpy.ndarray``
        An input image.
    rsp : :class:`sati.Responsibility`
        An initial guess of the responsibility.
    ...
    roi : ``numpy.ndarray``, default ``None``
        ...
        specifies a region of interest. Set pixels to be calculated
        to 1.  By default, the whole area is used for the calculation.

    Attributes
    ----------
    objective : :class:`sati.objective.Objective`
        Tracks the objective function value and convergence at each iteration.
    image : ``numpy.ndarray``
    ...
    prior : :class:`sati.distributions.vonMises`
        An estimated prior distribution.
    n_iter : ``int``
        Number of iterations run by the main EM loop to reach
    ...
    def __init__(
        self,
        image: np.ndarray,
        rsp: sati.Responsibility,
        dist: sati.distributions.Distribution,
        poly: sati.planes.Poly | None = None,
        decay: sati.planes.Decay | None = None,
        prior: sati.distributions.VonMises | None = None,
    ...
    def __validate(self, name: str, value) -> None:
    ...
    @property
    def rsp(self) -> sati.Responsibility:
        return self.__rsp

    @rsp.setter
    def rsp(self, value: sati.Responsibility) -> None:
        self.__validate("rsp", value)
        self.__rsp = deepcopy(value)

    @property
    def dist(self) -> sati.distributions.Distribution:
        return self.__dist

    @dist.setter
    def dist(self, value: sati.distributions.Distribution) -> None:
        self.__validate("dist", value)
        self.__dist = deepcopy(value)

    @property
    def poly(self) -> sati.planes.Poly | None:
        return self.__poly

    @poly.setter
    def poly(self, value: sati.planes.Poly | None) -> None:
        self.__validate("poly", value)
        self.__poly = deepcopy(value)

    @property
    def decay(self) -> sati.planes.Decay | None:
        return self.__decay

    @decay.setter
    def decay(self, value: sati.planes.Decay | None) -> None:
        self.__validate("decay", value)
        self.__decay = deepcopy(value)

    @property
    def prior(self) -> sati.distributions.VonMises | None:
        return self.__prior

    @prior.setter
    def prior(self, value: sati.distributions.VonMises | None) -> None:
    ...
    def __setup(self) -> None:
        ...
        self.__gamma = self.rsp.values.reshape((self.rsp.values.shape[0], -1))
        ...
        self.poly.setup(self.image.shape, None, self.roi)
    ...
    def __swap_axes(self, backward: bool = False) -> None:
        if self.decay is None:
            raise NoAttributeError("decay")
        self.image = _swap_axes(self.image, self.decay.orgdrct, backward)
        self.rsp.values = _swap_axes(self.rsp.values, self.decay.orgdrct, backward)
        if self.roi is not None:
            self.roi = _swap_axes(self.roi, self.decay.orgdrct, backward)
        if backward:
            self.subtracted = _swap_axes(self.subtracted, self.decay.orgdrct, backward)

    def __select_m_step(self) -> Callable[[], None]:
        if self.__method not in ["auto", "l-bfgs-b", "adam", "quick"]:
            raise ValueError("unknown method.")

        is_quick_available = all(
            [hasattr(self.dist, "update_quick"), self.decay is None, self.prior is None]
        )
    ...
    def __initial_step(self) -> None:
        ...
        self.objective.append(
            sati.objective.calc(self.dist, self.__t_subtracted, prior=self.prior)
        )
    ...
    def __m_step_quick(self) -> None:
        ...
        if self.poly is None:
            raise NoAttributeError("poly")
        if self.dist.loc is None:
            raise RuntimeError("dist.loc must be initialized before __m_step_quick.")
    ...
    def __m_step_ga_bfgs(self) -> None:
        """Do the M-step using the L-BFGS-B algorithm."""
        x0 = [c.x0() for c in self.__classes]
        self.__m_step_sizes = [x.size for x in x0]
        bounds = scipy.optimize.Bounds(
            np.concatenate([c.lb() for c in self.__classes], axis=None), np.inf
        )
        result = scipy.optimize.minimize(
            self.__m_step_ga_fun,
            np.concatenate(x0, axis=None),
            method="L-BFGS-B",
            jac=self.__m_step_ga_jac,
            bounds=bounds,
    ...
    def __m_step_ga_adam(self) -> None:
    ...
    def __m_step_ga_update_values(self, x: np.ndarray) -> None:
        if self.poly is None:
            raise NoAttributeError("poly")
    ...
    def __m_step_ga_fun(self, x: np.ndarray) -> float:
        self.__m_step_ga_update_values(x)
        return sati.objective.calc(self.dist, self.__t_subtracted, prior=self.prior) * (
            -1
        )

    def __m_step_ga_jac(self, x: np.ndarray) -> np.ndarray:
        if self.poly is None:
            raise NoAttributeError("poly")
        self.__m_step_ga_update_values(x)
        ...
        if self.prior is not None:
            if self.dist.loc is None:
                raise RuntimeError(
                    "dist.loc must be initialized before __m_step_ga_jac."
                )
    ...
    def __finalize(self) -> None:
        ...
        if self.roi is not None:
            self.__rsp.values = np.zeros((n_class, self.image.size))
            np.place(
                self.__rsp.values,
                np.broadcast_to(self.roi, self.__rsp.values.shape),
                self.__gamma,
            )
        else:
            self.__rsp.values = self.__gamma
        self.__rsp.values.resize((n_class,) + self.image.shape)
...
def _swap_axes(array: np.ndarray, orgdrct: str, backward: bool = False) -> np.ndarray:
    """Reorder a data array to the standard scan order by flipping and
    swapping axes.

    Forward (``backward=False``) converts from the original to the standard
    order; backward reverses this. Since flip and transpose do not commute,
    the swap is applied last for the forward direction and first for the
    backward direction.
    """
...
def _elapsed(elapsed: float) -> str:
...
class ArgumentTypeError(SatiError):
    ...
    def __str__(self):
        return (
            f"'{self.__name}' must be an instance of "
...
class NotConvergedWarning(SatiWarning):
    """A warning raised when the optimization did not reach the
    specified tolerance."""
